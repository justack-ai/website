...
export default function SubscriptionStatus({ tier, status, calculationsUsed, maxCalculations }: SubscriptionStatusProps) {
  if (status === "past_due") {
    return (
      <div className="inline-flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-sm text-amber-400">
        <span className="h-2 w-2 rounded-full bg-amber-400" />Payment issue — please update billing
      </div>
    );
  }
  ...
  if (tier === "free") {
    return (
      <div className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white/60">
        <span className="h-2 w-2 rounded-full bg-white/40" />Free tier — {calculationsUsed} of {maxCalculations} calculations used
      </div>
    );
  }
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-lg border border-teal-500/30 bg-teal-500/10 px-3 py-1.5 text-sm text-teal-400">
      <span className="h-2 w-2 rounded-full bg-teal-400" />{tier === "solo" ? "Solo" : "Firm"} plan — Active
    </div>
  );
}
