...
import type { CorpusIndex, CorpusIssue, ConfidenceTier, CorpusPendingAuthority } from "./types";
...
export function resolveIssueTier(issueId: string, jurisdiction: string): ConfidenceTier {
  const issue = getCorpusIssue(issueId);
  if (!issue) return 5;
  const variant = issue.jurisdiction_variants.find((v) => v.jurisdiction === jurisdiction);
  if (variant) return variant.confidence_tier;
  return issue.default_confidence_tier;
}

export function getPendingAuthoritiesForIssues(issueIds: string[]): CorpusPendingAuthority[] {
  const index = getCorpusIndex();
  const seen = new Set<string>();
  const results: CorpusPendingAuthority[] = [];

  for (const pa of index.global_pending_watch) {
    const affects = (pa as CorpusPendingAuthority & { affects_issues?: string[] }).affects_issues;
