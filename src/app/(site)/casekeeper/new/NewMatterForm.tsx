...
import type { MatterType, MatterJurisdiction } from "@/lib/casekeeper/types";
import { getDefaultWorkflowConfigId, getWorkflowConfigsForType } from "@/lib/casekeeper/configs";
...
export default function NewMatterForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const selectedMatterType = fd.get("matter_type") as MatterType;
    const body = {
      ...
      workflow_config_id:
        (fd.get("workflow_config_id") as string) ||
        getDefaultWorkflowConfigId(selectedMatterType),
    };
    ...
    try {
      const res = await fetch("/api/casekeeper/matters", {
        method: "POST",
      ...
      router.push(`/casekeeper/${data.matter.id}`);
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
      ...
      {(() => {
        const workflows = matterType ? getWorkflowConfigsForType(matterType as MatterType) : [];
        return workflows.length > 1 ? (
          <div>
            ...
            <select
              ...
              defaultValue={getDefaultWorkflowConfigId(matterType as MatterType)}
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"
            >
              {workflows.map((c) => (
